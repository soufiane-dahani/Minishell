...
/*   Updated: 2025/03/18 01:12:54 by sodahani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    t_ast_node **new_temp = ft_calloc(cmd_count + 1, sizeof(t_ast_node *));
    if (!new_temp)
        return NULL;
...
void process_commands(t_ast_node *current, t_ast_node ***commands, int *cmd_count)
{
    while (current)
    {
        if (current->type == PIPE) {            
            ...
            if (current->left && current->left->type == SIMPLE_CMD) {
                t_ast_node **temp = resize_commands(*commands, *cmd_count);
                if (!temp) {
            ...
            t_ast_node **temp = resize_commands(*commands, 1);
            if (!temp) {
                perror("calloc failed");
                free(*commands);
                exit(EXIT_FAILURE);
...
int **create_pipes_and_pids(int cmd_count, t_ast_node **commands, pid_t **pids)
{
    ...
    *pids = malloc(cmd_count * sizeof(pid_t));
    if (!*pids) {
        perror("malloc failed");
        i = 0;
        while (i < num_pipes) free(pipes[i++]);
        free(pipes);
        free(commands);
        exit(EXIT_FAILURE);
    }
...
void execute_commands(int cmd_count, t_ast_node **commands, int **pipes, pid_t *pids, char **envp)
{
    int num_pipes = cmd_count - 1;
    int i = 0;
    ...
    while (i < cmd_count) {
        ...
        if (pids[i] == -1) {
            perror("fork failed");
            int j = 0;
            while (j < num_pipes) free(pipes[j++]);
            free(pipes);
            free(commands);
            free(pids);
            exit(EXIT_FAILURE);
...
void execute_pipe(t_ast_node *node, char **envp)
{
    ...
    int **pipes = create_pipes_and_pids(cmd_count, commands, &pids);
    if (!pipes) {
