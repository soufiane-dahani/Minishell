...
/*   Updated: 2025/04/22 16:37:56 by sodahani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../minishell.h"

static int	is_valid_env_assignment_env(char *arg)
{
	int	i;
...
static int	count_env_vars(char **envp, char **cmd, int *env_len, int *add_len)
{
	...
	*add_len = 0;
	while (envp && envp[*env_len])
		(*env_len)++;
	while (cmd[*add_len + 1] && is_valid_env_assignment_env(cmd[*add_len + 1]))
		(*add_len)++;
...
static void	copy_variables(char **new_env, char **envp, char **cmd, int env_len)
{
	...
	j = 0;
	while (cmd[j + 1] && is_valid_env_assignment_env(cmd[j + 1]))
	{
...
int	my_env(char **cmd, char **envp)
{
	...
	if (cmd[1] && !is_valid_env_assignment_env(cmd[1]))
	{
